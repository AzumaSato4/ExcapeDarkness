...
public class RoomManager : MonoBehaviour
{
    ...
    public GameObject[] items = new GameObject[5];   //5つのアイテムのプレハブ
    ...
    public static string toRoomNumber = "fromRoom1";     //Playerが配置されるべき場所
    ...
    void StartKeysPosition()
    {
        ...
        //Key2およびKey3の対象スポット
        GameObject keySpot;
        GameObject obj; //生成したKey2およびKey3が入る予定
    ...
    void StartItemsPosition()
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            ...
            do
            {
                unique = true;  //問題がなければそのままループを抜ける予定
                rand = Random.Range(1, (itemSpots.Length + 1)); //1番からスポット数の番号をランダムで取得

                //すでにランダムに取得した番号がどこかのスポットとして割り当てられていないか、itemsPositionNumber配列の状況を全点検
            ...
            foreach (GameObject spots in roomSpots)
            {
                if (spots.GetComponent<RoomSpot>().spotNum == rand)
                {
                    ...
                    DoorSetting(
                        ...
                        "Main", //行先となるシーン名
                        false,  //ドアの解錠の状況
                        DoorDirection.down, //子の出入り口に戻った時のプレイヤーの配置
                        messages[i]
                        );
                }
            }
    ...
    {
        //Key1が未取得だったら
        if (!GameManager.keyPickedState[0])
        {
            //全Keyスポットの取得
            GameObject[] keySpots = GameObject.FindGameObjectsWithTag("KeySpot");
        ...
        //Key2が未取得だったら
        if (!GameManager.keyPickedState[1])
        {
            //Key2スポットの取得
        ...
        //Key3が未取得だったら
        if (!GameManager.keyPickedState[2])
        {
            //Key3スポットの取得
            ...
            foreach (GameObject spots in roomSpots)
            {
                if (spots.GetComponent<RoomSpot>().spotNum == doorsPositionNumber[i])
                {
                    ...
                    DoorSetting(
                        ...
                        "Room" + (i + 1),   //そこの出入り口に触れたときにどこに行くのか
                        "Main", //行先となるシーン名
                        GameManager.doorsOpenedState[i],  //ドアの解錠の状況をstaticから読み取る
                        DoorDirection.down, //子の出入り口に戻った時のプレイヤーの配置
                        messages[i]
                        );
                }
