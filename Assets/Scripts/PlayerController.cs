...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("プレイヤーの角度計算用")]
    public float angleZ = -90;  //プレイヤーの角度計算用

    [Header("オン/オフ対象のスポットライト")]
    ...
    bool isViartual;    //バーチャルパッドを触っているかどうかの判断フラグ
    ...
    public void Move()
    {
        if (!isViartual)    //バーチャルパッドを触っていないのであれば
        {
    ...
    void Animation()
    {
        ...
        if (axisH != 0 || axisV != 0)
        {
            ...
            //angleZを利用して方角を決める　パラメーターdirection　int型
    ...
    //バーチャルパッドの入力に対応するメソッド
    public void SetAxis(float virH, float virV)
    {
        //どちらかの引数に値が入っていればバーチャルパッドが使われた
        if (virH != 0 || virV != 0)
        {
            isViartual = true;
            axisH = virH;
